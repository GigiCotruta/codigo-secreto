...
export function RoleSelector({ players, currentRole, onSelectRole, disabled }: RoleSelectorProps) {
  const options: Array<{ role: PlayerRole; label: string; colorClass: string }> = [
    ...
    { role: "spectator", label: "Espectador", colorClass: "border-slate-300 bg-slate-50 text-slate-700" },
  ];

  return (
    <div className="grid gap-2 sm:grid-cols-3" role="group" aria-label="Selección de rol">
      {options.map((option) => {
        const occupied = option.role !== "spectator" && hasRole(players, option.role) && currentRole !== option.role;
        return (
          <button
            key={option.role}
            type="button"
